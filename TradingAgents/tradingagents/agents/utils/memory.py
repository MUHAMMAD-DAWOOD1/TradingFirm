...
class TradingMemoryLog:
    ...
    def get_past_context(self, ticker: str, n_same: int = 5, n_cross: int = 3) -> str:
        ...
        same, cross = [], []
        target_cross = n_cross + 2  # Boost cross-ticker lessons for cold-starts if same is empty
        for e in reversed(entries):
            if len(same) >= n_same and len(cross) >= target_cross:
                break
            if e["ticker"] == ticker and len(same) < n_same:
                same.append(e)
            elif e["ticker"] != ticker and len(cross) < target_cross:
                cross.append(e)

        # Trim cross if same was found
        if same:
            cross = cross[:n_cross]
