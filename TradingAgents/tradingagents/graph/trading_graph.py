...
class TradingAgentsGraph:
    ...
    def __init__(
        ...
        cheap_model = self.config.get("cheap_think_llm")
        if cheap_model:
            ...
            self.cheap_thinking_llm = cheap_client.get_llm()
        else:
            self.cheap_thinking_llm = quick_client.get_llm()
        ...
        self.graph_setup = GraphSetup(
            ...
            cheap_thinking_llm=self.cheap_thinking_llm,
        )
    ...
    def resolve_all_pending() -> int:
        """Resolve all pending entries across all tickers in the memory log.
        ...
        tickers = set(e["ticker"] for e in pending)
        for ticker in tickers:
            self._resolve_pending_entries(ticker)
        return len(pending)

    def propagate_with_signal(
        self,
        company_name: str,
        trade_date: str,
        signal_data: dict,
        ...
        """Run the graph with an external trading signal for verification and confluence analysis."""
                ...
                if chunk["messages"]:
                    ...
                    if signature != last_printed:
                        try:
                            ...
                            # Fallback for Windows console character mapping (cp1252)
                            try:
                                print(str(getattr(msg, "content", "")).encode("ascii", "replace").decode("ascii"))
                            except Exception:
                                pass
