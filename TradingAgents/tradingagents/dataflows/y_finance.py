...
def get_YFin_data_online(
    ...
    if data.empty:
        try:
            data = yf_retry(lambda: ticker.history(period="5y"))
        except Exception:
            data = pd.DataFrame()

    if data.empty and canonical in ("GC=F", "XAUUSD"):
        for alt in ["XAUUSD=X", "GLD"]:
            try:
                data = yf_retry(lambda: yf.Ticker(alt).history(period="5y"))
                if not data.empty:
                    break
            except Exception:
                continue
