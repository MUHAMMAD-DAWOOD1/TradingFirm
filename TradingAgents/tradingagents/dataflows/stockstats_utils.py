...
def load_ohlcv(symbol: str, curr_date: str) -> pd.DataFrame:
    ...
    if data is None:
        ...
        # Fallback 1: Use Ticker history(period="5y") if download was empty or failed
        if downloaded.empty or "Close" not in downloaded.columns:
            try:
                hist = yf_retry(lambda: yf.Ticker(canonical).history(period="5y", auto_adjust=True))
                if not hist.empty:
                    downloaded = _ensure_date_column(hist.reset_index())
            except Exception:
                pass

        # Fallback 2: For Gold/Silver/Metals, try spot currency pair (XAUUSD=X) or ETF (GLD) if futures (GC=F) failed
        if downloaded.empty or "Close" not in downloaded.columns:
            alt_candidates = []
            if canonical in ("GC=F", "XAUUSD"):
                alt_candidates = ["XAUUSD=X", "GLD", "GC=F"]
            elif canonical in ("SI=F", "XAGUSD"):
                alt_candidates = ["XAGUSD=X", "SLV", "SI=F"]

            for alt in alt_candidates:
                if alt == canonical:
                    continue
                try:
                    hist = yf_retry(lambda: yf.Ticker(alt).history(period="5y", auto_adjust=True))
                    if not hist.empty:
                        downloaded = _ensure_date_column(hist.reset_index())
                        logger.info("Fell back from %s to %s for market data", canonical, alt)
                        break
                except Exception:
                    continue
