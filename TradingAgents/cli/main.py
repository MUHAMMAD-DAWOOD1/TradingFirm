...
def get_user_selections():
    ...
    # Step 2b: Market Instrument Type
    console.print(
        create_question_box(
            "Step 2b: Market Instrument Type",
            "Select Futures/Margin, Spot, or Forex execution mode"
        )
    )
    selected_market_type = select_market_type()

    # Step 2c: Strategy Horizon
    console.print(
        create_question_box(
            "Step 2c: Strategy Horizon",
            "Select Scalping, Intraday, Swing, or Position trading style"
        )
    )
...
def run_analysis(checkpoint: bool | None = None):
    ...
    with Live(layout, refresh_per_second=4):
        ...
        init_agent_state = graph.propagator.create_initial_state(
            ...
            market_type=selections.get("market_type", "futures"),
            trading_horizon=selections.get("trading_horizon", "intraday"),
        )
...
@app.command(name="resolve-all")
def resolve_all():
    """Batch resolve all pending decision log entries across all tickers."""
    from tradingagents.agents.utils.memory import TradingMemoryLog

    memory_log = TradingMemoryLog(DEFAULT_CONFIG)
    pending = memory_log.get_pending_entries()
    if not pending:
        console.print("[green]No pending log entries to resolve.[/green]")
        return

    console.print(f"[cyan]Resolving {len(pending)} pending memory log entry/entries...[/cyan]")
    ta = TradingAgentsGraph()
    n = ta.resolve_all_pending()
    console.print(f"[bold green]✓ Successfully resolved {n} pending entry/entries![/bold green]")
