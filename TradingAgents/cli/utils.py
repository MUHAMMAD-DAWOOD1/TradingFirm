...
def select_market_type() -> str:
    """Prompt the user to select their market instrument type."""
    choice = questionary.select(
        "Select Market Instrument Type:",
        choices=[
            "Futures / Margin (Leveraged Long/Short, Funding, TP/SL in Pips/Ticks)",
            "Spot Trading (1:1 Asset Ownership, Long-Only, No Leverage)",
            "Forex Spot / CFD (Currency & Commodity CFDs)",
        ],
        style=questionary.Style([("highlighted", "fg:cyan bold")]),
    ).ask()
    if not choice or choice.startswith("Futures"):
        return "futures"
    if choice.startswith("Spot"):
        return "spot"
    return "forex"


def select_trading_horizon() -> str:
    """Prompt the user to select their strategy horizon / time style."""
    choice = questionary.select(
        "Select Trading Strategy & Horizon:",
        choices=[
            "Scalping (M1/M5/M15 - Quick entries, 5-20 pips/ticks targets, ultra-tight SL)",
            "Intraday (M15/H1 - Close positions same day, medium targets)",
            "Swing Trading (H4/D1 - Hold for 2 to 7 days, wide TP/SL)",
            "Position Trading (Daily/Weekly - Hold for weeks/months, long-term trend)",
        ],
        style=questionary.Style([("highlighted", "fg:cyan bold")]),
    ).ask()
    if not choice or choice.startswith("Scalping"):
        return "scalping"
    if choice.startswith("Intraday"):
        return "intraday"
    if choice.startswith("Swing"):
        return "swing"
    return "position"
